//! SHA-256 manual (FIPS 180-4), streaming, nol dependency.
//! Correctness dikunci oleh vektor NIST di #[cfg(test)] bawah.
//! Nol panic: hanya operasi aritmetika wrapping + indexing dalam batas.
...
impl Sha256 {
    ...
    pub fn finalize(mut self) -> [u8; 32] {
        ...
        let mut i = 0;
        while i < tail.len() {
            compress(&mut self.state, &tail[i..i + 64]);
            i += 64;
        }
        let mut out = [0u8; 32];
        for (j, w) in self.state.iter().enumerate() {
            out[j * 4..j * 4 + 4].copy_from_slice(&w.to_be_bytes());
        }
        out
    }

    pub fn hex_finalize(self) -> String {
        hex(&self.finalize())
    }
}
